...
use App\Http\Requests\ImpleRegu\DetailRequest;
use App\Http\Requests\ImpleRegu\ListRequest;
use App\Http\Requests\ImpleRegu\SaveRequest;
use App\Http\Requests\ImpleRegu\UpdateRequest;
use App\Repositories\Policy\PolicyRepository;
use App\Support\Collection;
use Illuminate\Support\Facades\DB;

class ImpleReguController extends Controller
{
    ...
    public function store(SaveRequest $request)
    {
        ...
        $data['obj_type'] = OBJ_TYPE['imple_regu'];
    ...
    public function relationPolicyInsert($data, $id)
    {
        ...
        $type = $this->policyService->typeHasRelation(OBJ_TYPE['imple_regu']);
        ...
        if (!empty($relation)) {
            $this->policyService->relationRelationInsert($relation, $id, OBJ_TYPE['imple_regu']);
        }
    }
    ...
    {
        $this->policyService->relationIndustryUpdate($data['industry']??[], $id, OBJ_TYPE['imple_regu']);
        $this->policyService->conclusionUpdate($data['conclusion'], $id);
        $this->policyService->itemUpdate($data['item'], $id);

        $this->policyService->govAgenUpdate($data['gov_agen']??[], $id,OBJ_TYPE['imple_regu']);
        $this->summarizeService->relationSummarizeUpdate($data['summarize']??[], $id);
        $this->policyService->relationFileUpdate($data['file']??[], $id,OBJ_TYPE['imple_regu']);
    ...
    public function relationPolicyUpdate($data,$id)
    {
        ...
        $this->policyService->relationRelationUpdate($relation, $id, OBJ_TYPE['imple_regu']);
    }

    public function list(ListRequest $request)
    {
        $param = $this->filter($request);
        $param['obj_type'] = OBJ_TYPE['imple_regu'];
    ...
    {
        $data = $this->policyService->impleReguDetail($request->input('id'));
