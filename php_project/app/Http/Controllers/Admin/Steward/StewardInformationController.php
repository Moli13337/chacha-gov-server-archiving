...
use App\Http\Requests\StewardInformation\DeleteRequest;
use App\Http\Requests\StewardInformation\DetailRequest;
use App\Http\Requests\StewardInformation\ListRequest;
use App\Http\Requests\StewardInformation\SaveRequest;
use App\Http\Requests\StewardInformation\UpdateRequest;
use App\Repositories\Steward\StewardInformationFileRepository;
...
class StewardInformationController extends Controller
{
    ...
    protected $stewardInformationFileRepository;

    public function __construct(StewardInformationRepository $repository,
                                StewardInformationFileRepository $stewardInformationFileRepository)
    {
        $this->repository = $repository;
        $this->stewardInformationFileRepository = $stewardInformationFileRepository;
    }
    ...
    public function storeFile($request, $id)
    {
        ...
        if (!empty($file)) {
            foreach ($file as $key => $value) {
                $file[$key]['steward_info_id'] = $id;
                $file[$key] = array_merge($file[$key], returnCreatedUpdatedAt());
            }
            $this->stewardInformationFileRepository->storeBatchRepository($file);
        }
    }
    ...
    public function updateFile($request, $id)
    {
        ...
        $list = $this->stewardInformationFileRepository->getList($id, ['id']);
        ...
        if (!empty($file)) {
            ...
            if (!empty($file)) {
                $log = [
                    ...
                    'subject_id' => $id,
                    'subject_type_id' => ACTIVITY_SUBJECT_TYPE['steward_information'],
        ...
        if ($deletes) {
            $this->stewardInformationFileRepository->deleteRepository($deletes);
            $log = [
    ...
    public function detail(DetailRequest $request)
    {
        ...
        $data['publish_staff_name'] = array_get($data['publish_staff'], 'name', '');
