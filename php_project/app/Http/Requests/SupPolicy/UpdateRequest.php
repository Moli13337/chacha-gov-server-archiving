...
class UpdateRequest extends BaseFormRequest
{
    public function rules()
    {
        return [
            ...
            'name' => ['required', 'string', Rule::unique(PolicyModel::TABLE_NAME)->ignore($this->input('id'))->where(function ($query){
                $query->whereNull('deleted_at');
            })],
            'doc_num' => ['nullable', 'string', Rule::unique(PolicyModel::TABLE_NAME)->ignore($this->input('id'))->where(function ($query){
                $query->whereNull('deleted_at');
            })],
            ...
            // 行业
            "industry" => ['nullable', 'array', new UniqueDirection()],
            ...
            // 政策概述
            'summarize' => ['nullable', 'array'],
            'summarize.*.name' => ['required', 'string', 'max:30'],
            'summarize.*.id' => ['nullable', 'integer'],
