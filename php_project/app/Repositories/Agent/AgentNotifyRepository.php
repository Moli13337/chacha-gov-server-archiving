...
class AgentNotifyRepository extends BaseRepository
{
    ...
    {
        return AgentNotifyModel::class;
    }

    public function detail($where, $column = ['*'])
    {
        $data = $this->model->select($column)->where($where)->with('file')->with('staff:id,name')->first();
        if (empty($data)) {
            return [];
        }
        $data = $data->toArray();
        $data['staff_name'] = array_get($data['staff'], 'name', '');
        unset($data['staff']);
        return $data;
    }
    ...
    public function detailByClient($where, $column = ['*'])
    {
        $data = $this->model->select($column)->where($where)->with('file:agent_notify_id,name,save_url')->first();
        if (empty($data)) {
            return [];
        }
        $data = $data->toArray();
