...
//     只记录变更的值
    ...
    public function getDescriptionForEvent(string $eventName): string
    {
        ...
        $content = '';
        $base = array_get(trans('constant.activity_subject_type'), ACTIVITY_SUBJECT_TYPE['agent_complaint'], '');

        if ($type == ACTIVITY_TYPE['created']) {
            if (isset($this->attributes['is_dispose']) && $this->attributes['is_dispose'] == IS_DISPOSE['staff']) {
                $content = trans('validation.custom.dispose');
            } else {
                $content = $base;
            }
            ...
            $columnName = trans('mysqlColumn.agentComplaint');
            $temp_content = [];
    ...
    public function tapActivity(Activity $activity, $eventName)
    {
        ...
        $activity->subject_type_id = ACTIVITY_SUBJECT_TYPE['agent_complaint'];
